...
interface UseHabitatMessagesState {
...
interface UseHabitatMessagesOptions {
  limit?: number;
  autoRefresh?: boolean;
...
export function useHabitatMessages(
  ...
  const [state, setState] = useState<UseHabitatMessagesState>({
    messages: [],
    loading: false,
    loadingMore: false,
    sending: false,
  ...
  const fetchMessages = useCallback(
    async (reset = false) => {
      ...
      try {
        const newMessages = await habitatsService.getHabitatMessages(
          habitatId,
          userId,
          limit,
          currentOffset
  ...
  const sendMessage = useCallback(
    async (content: string) => {
      ...
      try {
        const newMessage = await habitatsService.sendMessage(
          habitatId,
          userId,
          content
        );
  ...
  }, [habitatId, userId, fetchMessages, autoRefresh, refreshInterval]);
  ...
  return {
    ...
    // Real-time helpers (for use with useRealtimeChat)
    addMessage,
    updateMessage,
    removeMessage,
  };
