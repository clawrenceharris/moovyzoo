...
/**
 * Search for movies and tv by title
 * Uses /search/multi endpoint
 */
export async function searchMulti(title: string): Promise<TMDBSearchResult[]> {
  if (!title.trim()) {
...
export function formatTVShowForDisplay(
  ...
  if ("networks" in tvShow && tvShow.networks && tvShow.networks.length > 0) {
    const networkNames = tvShow.networks
  ...
  if ("credits" in tvShow && tvShow.credits) {
    const mainCast = tvShow.credits.cast
      .slice(0, 5)
      .map((actor) => actor.name)
      .join(", ");
    if (mainCast) {
  ...
  if (
    "content_ratings" in tvShow &&
    tvShow.content_ratings &&
    tvShow.content_ratings.results.length > 0
  ) {
