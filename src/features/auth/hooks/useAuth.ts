...
import { authUtils, sessionUtils, profileUtils } from '../utils/firebase'
...
export function useAuth() {
  ...
  useEffect(() => {
    const unsubscribe = authUtils.onAuthStateChange(
      async (firebaseUser: FirebaseUser | null) => {
        try {
          if (firebaseUser) {
            // Convert Firebase user to app User interface
            const user = authUtils.convertFirebaseUser(firebaseUser)
            ...
            if (profileExists) {
              await profileUtils.updateLastActive(firebaseUser.uid)
            }
  ...
  const updatePassword = useCallback(
    async (newPassword: string): Promise<void> => {
      ...
      try {
        await authUtils.updateUserPassword(newPassword)
      } catch (error) {
        const errorMessage =
  ...
  const reauthenticate = useCallback(
    async (password: string): Promise<void> => {
      ...
      try {
        await authUtils.reauthenticate(password)
      } catch (error) {
        const errorMessage =
  ...
  const isSessionValid = sessionUtils.isSessionValid()
