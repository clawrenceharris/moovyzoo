...
// Movie-related tools for MoovyZoo
export const getMovieRecommendation = tool((input) => {
  ...
  return `Based on your preferences for ${input.genre || 'great movies'}, I recommend "${randomMovie}" - it's a fantastic ${randomGenre} film that many MoovyZoo users love!`;
}, {
...
export const createAIAgent = () => {
  ...
  const agent = createReactAgent({
    ...
    messageModifier: `You are MoovyZoo's AI assistant, a friendly and knowledgeable companion for movie and TV enthusiasts. 
...
Remember: You're here to enhance the MoovyZoo experience and help users connect with great content!`
  });
...
export const configureLangSmith = () => {
  if (process.env.LANGCHAIN_API_KEY) {
    ...
    process.env.LANGCHAIN_PROJECT = process.env.LANGCHAIN_PROJECT || 'MoovyZoo-AI-Discussion';
  }
};